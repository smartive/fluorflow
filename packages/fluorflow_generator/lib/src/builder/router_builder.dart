  ...
  FutureOr<void> build(BuildStep buildStep) async {
    ...
    await for (final assetId in buildStep.findAssets(_allDartFilesInLib)) {
      ...
      for (final AnnotatedElement(:annotation, :element) in lib
        ...
        final ctor =
            element.children.cast<Element?>().firstWhere(
        ...
        pages[refer(
          routeEnum.name,
        ).property(element.displayName.camelCase).property('path')] = Method(
          (b) =>
              b
            ...
            Method(
              (b) =>
                  b
                    ...
                    ..types.addAll([if (withReturnFuture) refer('T')])
                    ..optionalParameters.addAll(
                      params.map(
                        (p) => Parameter(
                          (b) =>
                            ...
                            .call(
                              [refer('AppRoute.${displayName.camelCase}.path')],
                              {
                                ...params.isNotEmpty
                                    ? {
                                      'arguments': refer(
                                        '${displayName.pascalCase}Arguments',
                                      ).newInstance([], {
                                        for (final p in params)
                                          p.name: refer(p.name),
                                      }),
